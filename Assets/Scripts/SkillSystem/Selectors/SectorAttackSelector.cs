...
{	
	public class SectorAttackSelector : IAttackSelector
	{
	    public Transform[] SelectTarget(SkillData data, Transform skillTF)
	    {
	        ...
	        //判断攻击范围（圆形/扇形）
	        targets.FindAll(t =>
	            Vector3.Distance(t.position, skillTF.transform.position) <= data.attackDistance &&
	            Vector3.Angle(skillTF.forward, t.position - skillTF.transform.position) <= data.attackAngle / 2
	        );
	        //筛选角色
